  ...
  try {
    ...
    if (!freemiusResponse.ok) {
      ...
      if (freemiusResponse.status === 404) {
        throw new ApiError(
          404,
          "No Freemius customer was found for that email.",
        );
      }
    ...
    if (!isRecord(payload) || typeof payload.link !== "string") {
      ...
      throw new ApiError(
        502,
        "Freemius did not return a customer portal link.",
      );
    }
...
function addNextPath(link: string) {
  ...
  if (!nextPath.startsWith("/store/")) {
    console.warn(
      "Ignoring FREEMIUS_PORTAL_NEXT_PATH because it is not a store path.",
    );
    return link;
  }
